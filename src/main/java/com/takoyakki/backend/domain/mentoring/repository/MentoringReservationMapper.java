...
public interface MentoringReservationMapper {
    ...
    // 예약 생성
    void insertReservation(MentoringReservationDto reservation);

    // 특정 날짜 기준 예약 조회
    List<MentoringReservationDto> findReservationsByDate(@Param("date") String date);

    //특정 멘티의 예약 목록 조회 (진행 중 / 대기)
    List<MentoringReservationDto> findReservationsByMenteeId(@Param("menteeId") Long menteeId);

    // 특정 멘티의 지난 예약 내역 조회 (완료된 멘토링)
    List<MentoringReservationDto> findHistoryReservationsByMenteeId(@Param("menteeId") Long menteeId);
