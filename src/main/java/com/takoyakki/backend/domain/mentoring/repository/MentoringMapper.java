...
public interface MentoringMapper {
    ...
    // 전체 멘토링 조회 (페이징)
    List<MentoringRequestDto> findAllMentoring(@Param("offset") int offset, @Param("limit") int limit);

    // ID 기준 멘토링 상세 조회
    MentoringRequestDto selectMentoringById(@Param("id") Long id);

    // 특정 멘토의 멘토링 목록 조회
    List<MentoringRequestDto> findMentoringByMentorId(@Param("mentorId") Long mentorId);
